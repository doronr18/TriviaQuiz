...
import customizeIcon from '../img/slider.js'
...
export default function QuizCustomization(props) {
  ...
  return (
    <div className="preference-container">
      ...
      <div className="absolute right-[6px] top-[1px] text-6xl text-red-600">
      <ion-icon name="menu-outline" alt="customize icon" className={visible ? "custom-quizzical visible" : "custom-quizzical"} onClick={() => toggleCustomMenu()}></ion-icon>
      </div>

      {!quiz ? <form className={visible ? 'quiz-menu on-screen' : 'quiz-menu'}>
        <h1 className="quiz-menu-header">Settings</h1>
        <label className="text-red-600">Select Difficulty:</label>
        <select
          id='difficulty'
          className="input-box" 
        ...
        <label className="text-red-600">Select Category:</label>
        <select
          id="category"
          className="input-box" 
        ...
        <label className="text-red-600">Number of Questions:</label>
        <input
          type="number"
          id="number"
